...
findLeft p (ListZipper l x r) =
  case break p l of
    (_, Nil) -> IsNotZ
    (r', x' :. l') -> IsZ (ListZipper l' x' (r' ++ (x :. r)))
...
moveLeftN n l
  | n == 0 = IsZ l
  | n < 0 = moveRightN (n * (-1)) l
  | otherwise = moveLeftN (n - 1) -<< moveLeft l
...
moveRightN n l
  | n == 0 = IsZ l
  | n < 0 = moveLeftN (negate n) l
  | otherwise = moveRightN (n - 1) -<< moveRight l
...
insertPushLeft x' (ListZipper ls x rs) = ListZipper (x:.ls) x' rs
...
insertPushRight x' (ListZipper ls x rs) = ListZipper ls x' (x :. rs)
...
-- /Tip:/ Use @List#repeat@.
  ...
  (<*>) (ListZipper fls fx frs) (ListZipper als ax ars) =
    ListZipper (zipApply fls als) (fx ax) (zipApply frs ars)
    where zipApply = zipWith (\f a -> f a)
...
-- This implementation "visits" every possible zipper value derivable from a given zipper
-- (i.e. all zippers to the left and right).
--
...
instance Extend ListZipper where
  -- (<<=) :: (ListZipper a -> b) -> ListZipper a -> ListZipper b
  (<<=) f z =
    ListZipper ls (f z) rs
    where
      ls = unfoldr (moveWith moveLeft) z
      rs = unfoldr (moveWith moveRight) z
      moveWith move z' =
        case move z' of
          IsZ z'' -> Full (f z'', z'')
          _ -> Empty
