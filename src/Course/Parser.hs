...
list1 p =
  p `flbindParser` \a -> (a :.) `mapParser` list p
...
satisfy p =
  character `flbindParser` satisfy'
  where
    satisfy' c
      | p c = valueParser c
...
sequenceParser =
  foldRight (\p as -> p `flbindParser` \a -> mapParser (a :.) as) (valueParser Nil)
...
firstNameParser =
  upper `flbindParser` \c -> (c :.) `mapParser` list lower
...
surnameParser =
  upper
  `flbindParser` \c -> thisMany 5 lower
  `flbindParser` \ls -> list lower
  `flbindParser` \ls' -> valueParser $ c :. (ls ++ ls')
...
phoneParser =
  ...
  phoneBodyParser `flbindParser` \body ->
  is '#' `flbindParser` \_ ->
  valueParser (d :. body)
...
personParser =
  ...
  spaces1 >>> firstNameParser `flbindParser` \firstName' ->
  spaces1 >>> surnameParser `flbindParser` \surname' ->
  spaces1 >>> smokerParser `flbindParser` \smoker' ->
  spaces1 >>> phoneParser `flbindParser` \phone' ->
