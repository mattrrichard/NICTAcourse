...
hex :: Parser Char
hex =
  chr . reader <$> hexParser
  where
    hexParser = replicateA 4 (satisfy isHexDigit)
    reader s = readHex s ?? 0
...
eof =
  P isEof
  where
    isEof Nil = Result Nil ()
    isEof rest = ErrorResult $ ExpectedEof rest
...
betweenSepbyComma s e p =
  betweenCharTok s e (sepby p (charTok ','))
